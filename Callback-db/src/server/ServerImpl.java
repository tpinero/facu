...
public class ServerImpl
    ...
    public Boolean registerForCallback(ClientInterface callbackClientObject) throws RemoteException {
        if (!(clientList.contains(callbackClientObject))) {
            ...
            return this.giveResponseToClient(callbackClientObject, Boolean.TRUE);
        } else {
            return this.giveResponseToClient(callbackClientObject, Boolean.FALSE);
        }
    }

    private Boolean giveResponseToClient(ClientInterface client, Boolean clientSuccesfullyRegistered) throws RemoteException {
    ...
    public String getUser(String userIdentifier, ClientInterface callbackObj) throws RemoteException {
        try {
            ResultSet result = dbConnector.selectFromWhere("*", "user", "numberId", userIdentifier);
            if (result.next()) {
                ...
                user.setIdentifier(Long.valueOf(result.getLong("numberId")));
                user.setPhonenumber(Integer.valueOf(result.getInt("phonenumber")));
